...
//Calls the safe_move service to safely move the arm to the center position
void move_arm_center()
{
  ...
  //Calls the safe_move service and pass the requested joint angles
  if (!client.call(srv))
  {
...
//Continuously executes and reads the arm joint angles position
void joint_states_callback(const sensor_msgs::JointState js)
{
  ...
  //Defines a tolerance threshold to compare double values
  double tolerance = 0.0005;

  //Checks if the arm is moving by comparing its current joints position to its latest
...
void look_away_callback(const sensor_msgs::Image img)
{
  ...
  //Loops through each pixel in the image and check if its equal to the first one
...
int main(int argc, char** argv)
{
  ...
  //Defines a client service capable of requesting services from safe_move
  client = n.serviceClient<simple_arm::GoToPosition>("/arm_mover/safe_move");

  //Subscribes to /simple_arm/joint_states topic to read the arm joints position inside the joint_states_callback function
  ros::Subscriber sub1 = n.subscribe("/simple_arm/joint_states", 10, joint_states_callback);

  //Subscribes to rgb_camera/image_raw topic to read the image data inside the look_away_callback function
